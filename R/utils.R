...
pkg_resource = function(...) system.file(
  'rmarkdown', 'templates', 'slidepackR', 'resources', ..., package = 'slidepackR',
  mustWork = TRUE
)
...
check_builtin_css = function(theme) {
  ...
  stop(
    '"', invalid, '" is not a valid slidepackR theme', if (hint != "") hint else ".",
    " Use `slidepackR:::list_css()` to view all built-in themes.", call. = FALSE
  )
}
...
slide_context = function(ctx = rstudioapi::getSourceEditorContext()) {
  ...
  o = getOption('slidepackr.page_number.offset', 0L)
...
slide_navigate = function(ctx = rstudioapi::getSourceEditorContext(), message) {
  ...
  i = prose_index(x, warn = FALSE); x2 = x; if (length(i)) x2[-i] = ''
  s = grep('^---?$', x2); o = getOption('slidepackr.page_number.offset', 0L)
